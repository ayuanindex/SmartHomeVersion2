...
public class CustomerMqttConnect {
    ...
    private final static String mJsonFileName = "kongtiao.json";
    ...
    public void send(int powerSwitch) {
        if (txDataTemplateClient != null && txDataTemplateClient.isConnected()) {
            try {
                ...
                /*txDataTemplateClient.publish(" $thing/up/property/" + mProductId + "/" + mDevName, message, context);*/
    ...
    public void publish(JSONObject property) {
        txDataTemplateClient.propertyReport(property, null);
    }
